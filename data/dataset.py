...
class Text2MotionDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        if opt.is_train:
            ...
            # face (B, seq_len, 50)
            std[4 + (joints_num - 1) * 9 + joints_num * 3 + 4: 4 + (joints_num - 1) * 9 + joints_num * 3 + 4 + 50] = std[
                                                              4 + (joints_num - 1) * 9 + joints_num * 3 + 4: 4 + (joints_num - 1) * 9 + joints_num * 3 + 4 + 50] / 1.0            
            # foot contact (B, seq_len, 4)
            std[4 + (joints_num - 1) * 9 + joints_num * 3 + 4 + 50:] = std[
                                                              4 + (joints_num - 1) * 9 + joints_num * 3 + 4 + 50:] / opt.feat_bias
            assert 4 + (joints_num - 1) * 9 + joints_num * 3 + 50 + 4 == mean.shape[-1]
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        if False:
            print("set face param as zero")
            self.zero_face_param = True
        else:
            self.zero_face_param = False
    ...
    def __getitem__(self, item):
        ...
        if self.zero_face_param:
            assert motion.shape[1]==623+50
            motion[:,-54:-4] *= 0.0
